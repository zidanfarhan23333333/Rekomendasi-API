...
const {
  dapatkanRekomendasi,
  dapatkanRiwayat,
  getRankingGlobal, // ← tambah ini
} = require("../services/rekomendasi.service.js");

const postRekomendasi = async (req, res) => {
  const { cabor_id, user_id } = req.body;
  if (!cabor_id || !user_id)
    return res.status(400).json({ error: "cabor_id dan user_id diperlukan" });
  if (!Number.isInteger(Number(cabor_id)) || !Number.isInteger(Number(user_id)))
    return res
      .status(400)
      .json({ error: "cabor_id dan user_id harus angka bulat" });
...
const getRiwayat = async (req, res) => {
  ...
  if (isNaN(userId)) return res.status(400).json({ error: "id harus angka" });
...
const getRankingGlobalHandler = async (req, res) => {
  try {
    const data = await getRankingGlobal(); // ← sekarang sudah ter-import
    return res.json({ data });
  } catch (err) {
    console.error("[rekomendasiController] getRankingGlobal:", err);
